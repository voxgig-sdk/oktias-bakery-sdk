...
// OktiasBakery SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        OktiasBakerySDK $client,
        OktiasBakeryUtility $utility
    ): OktiasBakeryContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new OktiasBakeryContext($ctxmap, null);
    ...
    private static function fixctx(OktiasBakeryContext $ctx, OktiasBakerySDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?OktiasBakeryError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OktiasBakeryError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new OktiasBakeryResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new OktiasBakeryBaseFeature();
    ...
    {
        $hook_client = OktiasBakerySDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends OktiasBakeryBaseFeature {
            public $hook_fn = null;
            public function TestHook(OktiasBakeryContext $ctx): void
            {
    ...
    {
        $init_client = OktiasBakerySDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends OktiasBakeryBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new OktiasBakerySDK([
    ...
    {
        $blocked_client = new OktiasBakerySDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new OktiasBakerySpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = OktiasBakerySDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new OktiasBakeryOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = OktiasBakerySDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof OktiasBakeryError) ? $result->err->msg : (string)$result->err;
