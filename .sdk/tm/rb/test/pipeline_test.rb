...
class PipelineTest < Minitest::Test
  ...
  def spec_of(map = {})
    OktiasBakerySpec.new({ "step" => "s", "method" => "GET", "headers" => {} }.merge(map))
  end
  ...
  def test_make_response_guards_missing_spec_response_result
    ...
    ctx.result = OktiasBakeryResult.new({})
  ...
  def test_make_result_empty_list_yields_an_empty_resdata_array
    ...
    ctx.spec = spec_of
    ctx.result = OktiasBakeryResult.new({ "ok" => true, "resdata" => [] })
    result, err = @utility.make_result.call(ctx)
    assert_nil err
  ...
  def test_make_result_short_circuits_a_preset_result
    ctx = make_ctx
    preset = OktiasBakeryResult.new({ "ok" => true })
  ...
  def test_make_request_a_transport_error_tuple_lands_on_the_response
    boom = OktiasBakeryError.new("boom", "boom")
  ...
  def test_make_request_a_fetchdef_error_surfaces_as_a_response_error
    u = @client.get_utility
    u.make_fetch_def = ->(ctx) { [nil, OktiasBakeryError.new("fetchdef_boom", "boom")] }
    ctx = make_ctx(utility: u)
    ctx.spec = request_spec
    response, err = u.make_request.call(ctx)
    assert_nil err
  ...
  def test_done_returns_resdata_on_success
    ctx = make_ctx
    ctx.result = OktiasBakeryResult.new({ "ok" => true, "resdata" => 42 })
    assert_equal 42, @utility.done.call(ctx)
  end

  def test_done_raises_the_error_when_not_ok
    ctx = make_ctx
    ctx.result = OktiasBakeryResult.new({ "ok" => false })
    assert_raises(OktiasBakeryError) { @utility.done.call(ctx) }
  end

  def test_done_cleans_ctrl_explain_on_success
    ctx = make_ctx
    ctx.ctrl.explain = { "result" => { "err" => "x" } }
    ctx.result = OktiasBakeryResult.new({ "ok" => true, "resdata" => 7 })
  ...
  def test_make_error_returns_resdata_when_throw_is_disabled
    ...
    ctx.result = OktiasBakeryResult.new({ "ok" => false, "resdata" => "fallback" })
  ...
  def test_make_error_preserves_the_error_code
    ctx = make_ctx
    err = assert_raises(OktiasBakeryError) {
  ...
  def test_feature_add_appends_in_call_order
    ...
    client.features = []
    a = OktiasBakeryBaseFeature.new
    b = OktiasBakeryBaseFeature.new
  ...
  def auth_ctx(options, headers)
    ctx = OktiasBakeryContext.new({
      "client" => OptClient.new(options),
      "utility" => @utility,
      "opname" => "load",
    }, nil)
    ctx.spec = headers.nil? ? nil : OktiasBakerySpec.new({ "headers" => headers, "step" => "s" })
    ctx
  end
  ...
  def test_result_headers_with_non_hash_headers_yields_an_empty_map
    ctx = make_ctx
    ctx.response = OktiasBakeryResponse.new({ "status" => 200 })
    ctx.result = OktiasBakeryResult.new({})
  ...
  def test_result_body_skips_parsing_when_the_body_is_absent
    ctx = make_ctx
    ctx.response = OktiasBakeryResponse.new({
