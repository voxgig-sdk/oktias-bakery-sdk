...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = OktiasBakeryBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = OktiasBakerySDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = OktiasBakerySDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = OktiasBakerySDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = OktiasBakerySDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = OktiasBakerySpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = OktiasBakerySDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = OktiasBakeryResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = OktiasBakeryOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = OktiasBakerySDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = OktiasBakeryContext.new(ctxmap, nil)
    ...
    if ctxmap["spec"].is_a?(Hash)
      ctx.spec = OktiasBakerySpec.new(ctxmap["spec"])
    end

    # Handle result from JSON map
    if ctxmap["result"].is_a?(Hash)
      res_map = ctxmap["result"]
      ctx.result = OktiasBakeryResult.new(res_map)
      if res_map["err"].is_a?(Hash) && res_map["err"]["message"].is_a?(String)
        ctx.result.err = OktiasBakeryError.new("", res_map["err"]["message"])
      end
    end
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = OktiasBakeryResponse.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    OktiasBakeryError.new(code, msg)
  end
end
