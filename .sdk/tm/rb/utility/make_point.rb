...
module OktiasBakeryUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(OktiasBakeryError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      op.points.each do |p|
        point = p
        select_def = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(point, "select"))
