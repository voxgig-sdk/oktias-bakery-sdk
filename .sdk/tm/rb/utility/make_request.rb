...
module OktiasBakeryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OktiasBakeryResponse.new({})
    ...
    if fetch_err
      ...
      response = OktiasBakeryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OktiasBakeryResponse.new(fetched)
    else
