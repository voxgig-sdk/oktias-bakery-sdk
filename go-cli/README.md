...
boru-driven command-line client **and** interactive REPL for the OktiasBakery
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/oktias-bakery-cli)
make build

# 2. See usage (words, entities, env vars)
./oktias-bakery-cli --help

# 3. Provide credentials once, via the environment
export OKTIAS_BAKERY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./oktias-bakery-cli list product

# 5. Override the API base URL for a single call
OKTIAS_BAKERY_BASE=https://api.example.com ./oktias-bakery-cli list product

# 6. No arguments -> interactive REPL
./oktias-bakery-cli
oktias-bakery> list product
oktias-bakery> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/oktias-bakery-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/oktias-bakery-cli list product
   ```
...
Run with no arguments to open a REPL (prompt `oktias-bakery>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/oktias-bakery-cli`. |
...
(`./oktias-bakery-cli <expr>`) and interactively (the REPL), and expressions compose the
