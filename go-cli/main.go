// Command OktiasBakery-cli is a CLI + REPL for the OktiasBakery SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OKTIAS_BAKERY_APIKEY (api key), OKTIAS_BAKERY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OKTIAS_BAKERY_APIKEY carries the API key and
	// OKTIAS_BAKERY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OKTIAS_BAKERY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OKTIAS_BAKERY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOktiasBakerySDK(opts)
