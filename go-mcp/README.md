...
[MCP](https://modelcontextprotocol.io) server exposing the OktiasBakery SDK as
two agent tools — `oktias-bakery_list` and `oktias-bakery_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/oktias-bakery-mcp)
make build

# 2. Provide credentials via the environment
export OKTIAS_BAKERY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user oktias-bakery \
  -- /absolute/path/to/oktias-bakery-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./oktias-bakery-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// oktias-bakery_list: first page of records
{ "entity": "product" }
{ "entity": "product", "query": { } }

// oktias-bakery_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/oktias-bakery-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user oktias-bakery \
     -- "$PWD"/dist/*/oktias-bakery-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `oktias-bakery_list` and `oktias-bakery_load` tools now appear
   in new sessions. Ask the agent to *"list product using oktias-bakery"*
   and it calls `oktias-bakery_list` with `{"entity":"product"}`.
...
```sh
export OKTIAS_BAKERY_APIKEY=sk_live_xxx            # API key
export OKTIAS_BAKERY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `oktias-bakery_list` tool
...
### Call the `oktias-bakery_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `oktias-bakery_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `oktias-bakery_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OKTIAS_BAKERY_APIKEY` | API key sent with every request. |
| `OKTIAS_BAKERY_BASE` | Optional override of the API base URL. |
...
```sh
./oktias-bakery-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"oktias-bakery_load","arguments":{"entity":"product","query":{"id":1}}}}'
```
