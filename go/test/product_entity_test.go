...
func TestProductEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OKTIAS_BAKERY_TEST_PRODUCT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func productBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OKTIAS_BAKERY_TEST_PRODUCT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OKTIAS_BAKERY_TEST_PRODUCT_ENTID"])
	...
	if env["OKTIAS_BAKERY_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewOktiasBakerySDK(core.ToMapAny(mergedOpts))
	}
