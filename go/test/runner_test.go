...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OKTIAS_BAKERY_TEST_LIVE") == "TRUE" ||
		os.Getenv("OKTIAS_BAKERY_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OKTIAS_BAKERY_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OktiasBakerySDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OktiasBakeryError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OktiasBakerySDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OktiasBakeryError{Msg: msg, Code: code}
}
