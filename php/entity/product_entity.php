...
class ProductEntity
{
    ...
    {
        if ($args) {
            $this->_data = OktiasBakeryHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = OktiasBakeryHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws OktiasBakeryError on failure (item-5 convention).
     */
