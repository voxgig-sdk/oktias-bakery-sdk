...
// OktiasBakery SDK retry feature
...
class OktiasBakeryRetryFeature extends OktiasBakeryBaseFeature
{
    ...
    public function init(OktiasBakeryContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OktiasBakeryContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OktiasBakeryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OktiasBakeryContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
