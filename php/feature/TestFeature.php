...
// OktiasBakery SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OktiasBakeryTestFeature extends OktiasBakeryBaseFeature
{
    ...
    public function init(OktiasBakeryContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OktiasBakeryContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OktiasBakeryParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OktiasBakeryContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OktiasBakeryContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OktiasBakeryParam::call($ctx, $k);
