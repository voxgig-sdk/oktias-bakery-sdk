...
class FtCtrl extends OktiasBakeryControl
{
...
class FtClient
{
    ...
    private OktiasBakeryUtility $utility;

    public function __construct(OktiasBakeryUtility $utility, array $options)
    {
    ...
    public function get_utility(): OktiasBakeryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OktiasBakeryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OktiasBakeryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OktiasBakeryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OktiasBakeryConfig::make_feature($name);
    ...
    private static function buildUrl(OktiasBakerySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OktiasBakeryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OktiasBakeryResponse($fetched);
        }
    ...
    private function fail(OktiasBakeryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OktiasBakeryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
