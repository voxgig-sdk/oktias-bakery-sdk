...
// OktiasBakery SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class OktiasBakeryMakePoint
{
    public static function call(OktiasBakeryContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof OktiasBakeryError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = OktiasBakeryHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = OktiasBakeryHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
