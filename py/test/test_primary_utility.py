...
from oktiasbakery_sdk.utility.voxgig_struct import voxgig_struct as vs
...
from oktiasbakery_sdk.core.response import OktiasBakeryResponse
from oktiasbakery_sdk.core.operation import OktiasBakeryOperation
from oktiasbakery_sdk.core.error import OktiasBakeryError
from oktiasbakery_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return OktiasBakeryError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = OktiasBakerySDK.test(None, None)
        ...
        class TestInitFeature(OktiasBakeryBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(OktiasBakeryBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = OktiasBakerySDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OktiasBakerySpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = OktiasBakerySDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = OktiasBakeryOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = OktiasBakerySDK.test(None, setup_opts)
