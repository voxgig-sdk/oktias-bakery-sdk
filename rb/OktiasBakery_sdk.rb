...
require_relative 'OktiasBakery_types'


class OktiasBakerySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OktiasBakeryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OktiasBakeryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OktiasBakeryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OktiasBakeryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OktiasBakeryError.new(
        "#{op}_allow",
        "OktiasBakerySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OktiasBakeryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OktiasBakeryError.new(
        "graphql_error", "OktiasBakerySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OktiasBakerySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
