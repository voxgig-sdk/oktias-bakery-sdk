# OktiasBakery Ruby SDK



The Ruby SDK for the OktiasBakery API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Product` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = OktiasBakerySDK.test(testopts, sdkopts)
```
...
raise a `OktiasBakeryError` (a `StandardError` subclass), so wrap
...
```
rb/
├── OktiasBakery_sdk.rb       -- Main SDK module
...
The main module (`OktiasBakery_sdk`) exports the SDK class
