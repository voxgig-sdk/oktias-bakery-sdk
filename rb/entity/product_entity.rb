# OktiasBakery SDK Product entity
...
class ProductEntity
  ...
  def data_set(args)
    if args
      @_data = OktiasBakeryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OktiasBakeryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Product>, Array] the matching Product items; raises OktiasBakeryError on failure
