...
module OktiasBakeryFeatureHarness
  # True when this SDK was generated with the named feature.
  def self.has_feature?(name)
    f = OktiasBakeryConfig.shared_config["feature"]
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || OktiasBakeryFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless OktiasBakeryFeatureHarness.has_feature?(name)
        f = OktiasBakeryFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= OktiasBakeryFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(OktiasBakeryError)
        ...
        ctx.response = fetched.is_a?(Hash) ? OktiasBakeryResponse.new(fetched) : nil
...
class FeatureTest < Minitest::Test
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, OktiasBakeryError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, OktiasBakeryError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
