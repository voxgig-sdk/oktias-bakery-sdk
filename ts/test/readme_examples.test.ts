...
//     (\`new OktiasBakerySDK(...)\` / \`OktiasBakerySDK.test(...)\` become
//     \`OktiasBakerySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OktiasBakerySDK'
...
// rewrite real client construction (\`new OktiasBakerySDK(...)\` and any
// \`OktiasBakerySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OktiasBakerySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OktiasBakerySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
